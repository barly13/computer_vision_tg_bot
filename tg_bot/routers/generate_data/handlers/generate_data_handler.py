...
from ..keyboard import generate_settings_inline_kb
...
async def set_seed(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(f'{Emoji.Cancel} Введите одно или несколько целых чисел, например: 42 или 1,2,3')
        return
...
async def save_settings_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f'{Emoji.Save} Настройки сохранены:\n\n'
                                  ...
                                  f'Количество элементов на изображении: {data.get('elements_per_image', 100)}\n'
